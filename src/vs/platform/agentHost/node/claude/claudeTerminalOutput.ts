...
import { terminalOutputContent, terminalOutputPreview } from '../shared/terminalOutputContent.js';
...
export function createClaudeTerminalOutput(options: {
	...
	return terminalOutputContent(options.session, options.toolCallId, options.title, {
		...(output.preview !== undefined ? { preview: output.preview } : {}),
		truncated: true,
	});
}
